        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["infer", "report", "-o", self.infer_results_dir],
                    capture_output=True,
                    text=True,
                    check=False
                )
...
def process_infer_output(infer_dir: str) -> List[Dict[str, Any]]:
    """
...
def save_processed_results(results: List[Dict[str, Any]], output_file: str) -> bool:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument('--infer-dir', default='output/infer-out', help='Path to Infer output directory')
    parser.add_argument('--output-file', default='output/infer_processed.json', help='Path to save the processed results')
    args = parser.parse_args()
    
    save_processed_results(process_infer_output(args.infer_dir), args.output_file)
