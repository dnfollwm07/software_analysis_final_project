...
#include "../src/config_store_uninit_var.h"
using ConfigStoreImpl = ConfigStoreUninitVar;
...
class ConfigStoreUninitVarTest : public ::testing::Test {
protected:
    void SetUp() override {
        // 在每个测试前设置
    }
...
// 测试基本整数存储和检索
TEST_F(ConfigStoreUninitVarTest, IntStorage) {
...
// 测试基本浮点数存储和检索
TEST_F(ConfigStoreUninitVarTest, FloatStorage) {
    ConfigStoreImpl config;
...
// 测试基本字符串存储和检索
TEST_F(ConfigStoreUninitVarTest, StringStorage) {
    ConfigStoreImpl config;
...
// 测试基本向量存储和检索
TEST_F(ConfigStoreUninitVarTest, VectorStorage) {
    ConfigStoreImpl config;
...
// 测试不存在的键访问（整数）
TEST_F(ConfigStoreUninitVarTest, NonexistentInt) {
...
// 测试不存在的键访问（浮点数）
TEST_F(ConfigStoreUninitVarTest, NonexistentFloat) {
...
// 测试getInt中的类型不匹配
TEST_F(ConfigStoreUninitVarTest, TypeMismatchInt) {
    ConfigStoreImpl config;
...
// 测试buffer处理（边界内索引）
TEST_F(ConfigStoreUninitVarTest, BufferProcessing) {
    ConfigStoreImpl config;
    // 使用边界内的有效索引应该成功执行
    bool result = true;
    try {
...
// 测试buffer处理（越界索引）
TEST_F(ConfigStoreUninitVarTest, BufferProcessingOutOfBounds) {
    ConfigStoreImpl config;
...
// 测试set方法中的内存泄漏
TEST_F(ConfigStoreUninitVarTest, MemoryLeakOnOverwrite) {
...
// 测试sumBuffer方法
TEST_F(ConfigStoreUninitVarTest, SumBuffer) {
    ConfigStoreImpl config;
    config.processBuffer(0, 1);
    config.processBuffer(1, 2);
    int sum = config.sumBuffer(0, 2);
    EXPECT_EQ(sum, 3) << "sumBuffer(0, 2) should be 3";
}

// 测试sumBuffer方法（越界索引）
TEST_F(ConfigStoreUninitVarTest, SumBufferOutOfBounds) {
    // 由于以下原因，预期此测试会失败：
    // 1. 未初始化的缓冲区值
    // 2. 循环中的越界错误
