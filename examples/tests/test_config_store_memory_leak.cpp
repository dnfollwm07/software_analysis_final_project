...
#include "../src/config_store_memory_leak.h"
using ConfigStoreImpl = ConfigStoreMemoryLeak;
...
class ConfigStoreMemoryLeakTest : public ::testing::Test {
protected:
    void SetUp() override {
        // 在每个测试前设置
    }
...
// 测试基本整数存储和检索
TEST_F(ConfigStoreMemoryLeakTest, IntStorage) {
...
// 测试基本浮点数存储和检索
TEST_F(ConfigStoreMemoryLeakTest, FloatStorage) {
    ConfigStoreImpl config;
...
// 测试基本字符串存储和检索
TEST_F(ConfigStoreMemoryLeakTest, StringStorage) {
    ConfigStoreImpl config;
...
// 测试基本向量存储和检索
TEST_F(ConfigStoreMemoryLeakTest, VectorStorage) {
    ConfigStoreImpl config;
...
// 测试不存在的键访问（整数）
TEST_F(ConfigStoreMemoryLeakTest, NonexistentInt) {
...
// 测试不存在的键访问（浮点数）
TEST_F(ConfigStoreMemoryLeakTest, NonexistentFloat) {
...
// 测试getInt中的类型不匹配
TEST_F(ConfigStoreMemoryLeakTest, TypeMismatchInt) {
    ConfigStoreImpl config;
...
// 测试buffer处理（边界内索引）
TEST_F(ConfigStoreMemoryLeakTest, BufferProcessing) {
    ConfigStoreImpl config;
    // 使用边界内的有效索引应该成功执行
    bool result = true;
    try {
...
// 测试buffer处理（越界索引）
TEST_F(ConfigStoreMemoryLeakTest, BufferProcessingOutOfBounds) {
    ConfigStoreImpl config;
...
// 测试set方法中的内存泄漏
TEST_F(ConfigStoreMemoryLeakTest, MemoryLeakOnOverwrite) {
...
// 测试sumBuffer方法
TEST_F(ConfigStoreMemoryLeakTest, SumBuffer) {
    ConfigStoreImpl config;
...
// 测试sumBuffer方法（越界索引）
TEST_F(ConfigStoreMemoryLeakTest, SumBufferOutOfBounds) {
    // 由于以下原因，预期此测试会失败：
    // 1. 未初始化的缓冲区值
    // 2. 循环中的越界错误
