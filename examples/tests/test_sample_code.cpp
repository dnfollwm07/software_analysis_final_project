...
// 测试processData函数
TEST_F(ConfigStoreTest, ProcessData) {
    // 此测试检查processData函数的错误：
    // - 空指针检查
    // - 边界错误
    // - 越界访问
    // - 内存泄漏
    ...
    // 方法2：验证边界检查的其他迹象（返回值、状态等）
    // 这部分代码取决于修复后的实现方式
}

// 测试processData中的空指针处理
TEST_F(ConfigStoreTest, ProcessDataNullPtr) {
    // 方法1：如果实现添加了空指针检查并抛出异常
    try {
        processData(nullptr, 5);
        
        // 如果没有抛出异常，则检查其他指示空指针检查的迹象
        
        // 由于原始实现没有空指针检查，这个测试可能无法正确验证
        FAIL() << "应该检测到空指针并进行处理";
    } catch (const std::exception& e) {
        // 如果实现添加了空指针检查并抛出异常
        SUCCEED();
